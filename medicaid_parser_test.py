...
# MySQL connection
# con = mysql.connector.connect(user='ccgremote', password='ncsuball', host='ccgazrbpo1.cumberlandcg.com',
#                               database='medicaid')
...
holder = []
# replace special characters in raw text file
with codecs.open(file, 'r', encoding='utf-8') as input_file:
    for r in input_file:
        if u"—" in r or u"–" in r:
            r = r.replace(u"—", '-').replace(u"–", '-')
        holder.append(r)

for i in range(1, len(holder)):
    if 'Previous' in str(holder[i]) and 'Current' in str(holder[i + 1]):
        stacked_covenant_type = True
        holder[i - 1] = holder[i - 1][:-2] + " " + holder[i + 1][8:]
        temp_line = str(holder[i - 1])
        insertion_index = temp_line.rfind(" ")
        temp_line = "".join((temp_line[:insertion_index], " 0.00 ", temp_line[insertion_index + 1:]))
        holder[i - 1] = temp_line
print 'checkpoint -3'
holder = [x for x in holder if 'Previous' not in x and 'Current' not in x]

with codecs.open(file, 'wb', encoding='utf-8') as clean_out:
    # ~ print clean_out
    for r in holder:
        clean_out.write(r)
...
for row in lines:
    ...
    try:
        ...
        if logic['prior_quarter_id']:
            for id in str(logic['prior_quarter_id']).split('|'):
                if id in row_clean:
                    prior_qtr_flag = 'y'
                    print 'PQ'

        if 'MCOU' in row_clean or 'MMCO' in row_clean or 'MCO' in row_clean:
        ...
        if (ura_detail_check or ndc_detail_check) and 'TOTAL' not in row_clean:
            detail_row_flag = 'y'
        ...
        if detail_row_flag == 'y':
            ...
            if prior_qtr_flag == 'y':
                ...
                if logic['detail_qtr']:
                    ...
                    # print util_qtr_o, util_qtr, row_clean
            ...
            # remove garbage characters before ndc
            if not row_clean[0].isdigit():
                ndc_dirty = row_clean[:5]
                non_ndc_char = re.compile(r'[^\d-]+')
                ndc_clean = non_ndc_char.sub('', ndc_dirty)
                row_clean = ndc_clean + row_clean[5:]
            print 'checkpoint 2'
            ...
            if prior_qtr_flag == 'n':
                ...
                print 'true3'
                if logic['pq_prod_desc'] != 'n' and re.search('[a-zA-Z]', row_clean):
                    print 'true4'
                    # identifies product name start position with first space
                    start = row_clean.index(' ')
            ...
            if logic['notes']:
                ...
                if 'covenant' in str(logic['notes']) and stacked_covenant_type:
                    ...
                    if logic['pq_exp_col']:
                        print 'True2'
                        if prior_qtr_flag == 'y' and len(info_list) < int(logic['pq_exp_col']):
                            print 'True3'
                            info_list.insert(3, '0.00')
                            info_list.insert(4, '0.00')
            ...
            # Checking for non-detil rows at the end of covenant type invoices
            ignore_qtr_ids = {'1020', '2020', '3020', '4020'}
            temp_ndc = str(ndc).strip().replace('-', '')
            ignore_qtr_line = False
            for id in ignore_qtr_ids:
                if id in temp_ndc and len(temp_ndc) <= 6:
                    ignore_qtr_line = True

            # quality check - ndc field
            if len(str(ndc).strip().replace('-', '')) != 11:
                if ignore_qtr_line:
                    raise Exception("Non detail row ignored")
            ...
            for idx, item in enumerate(info_list):
                ...
                if str(item).count(".") > 1:
                    ...
                    info_list[idx] = item
                if str(item).count(',') > 1 and '.' not in str(item):
                    last_index = item.rfind(',')
                    item = item[:last_index - 1] + '.' + item[last_index:]
                    info_list[idx] = item
                item = item.replace(',', '')
                info_list[idx] = item
            # print row_clean
            # # remove extra fields in info list due to whitespace
            # if len(previous_info_list) != len(info_list):
            #     for i in range(len(previous_info_list)):
            #         if '.' in previous_info_list[i] and '.' not in info_list[i]:
            #             info_list[i] = info_list[i] + info_list[i + 1]
            #             del info_list[i + 1]
        ...
        print row_clean
        print str(e)
        if error_count == 0 and first_time:
            errors.append(str(logic['notes']))
            first_time = False
        if 'TOTAL' not in row_clean and len(str(row_clean)) > 50:
            error_count += 1
...
# with open('log.txt', 'ab') as output_file:
with open('F:\\sharefile\\Shared Folders\\mft\\out\\medicaid\\4 - cms_format\\test\\log.txt', 'ab') as output_file:
    output_file.write(
...
tech_error_file = 'F:\\sharefile\\Shared Folders\\mft\\out\\medicaid\\4 - cms_format\\test\\%s_%s_%s_%s_TECHNICAL_ERRORS.txt' % (
    inv_qtr, labeler, state, type)
# tech_error_file = '%s_%s_%s_%s_TECHNICAL_ERRORS.txt' % (inv_qtr, labeler, state, type)
error_file = 'F:\\sharefile\\Shared Folders\\mft\\out\\medicaid\\4 - cms_format\\test\\%s_%s_%s_%s_ERRORS.txt' % (
    inv_qtr, labeler, state, type)
...
with open('F:\\sharefile\\Shared Folders\\mft\\out\\medicaid\\4 - cms_format\\test\\%s_%s_%s_%s_csv.txt' % (
...
with open('F:\\sharefile\\Shared Folders\\mft\\out\\medicaid\\4 - cms_format\\test\\%s_%s_%s_%s_cms.txt' % (
        inv_qtr, labeler, state, type), 'wb') as output_file:
    for r in cms:
        output_file.write(r + '\r\n')
